...
# $db_username = 'mail';
if (-f "/etc/webapps/kopano-postfixadmin/fetchmail.conf") {
	require "/etc/webapps/kopano-postfixadmin/fetchmail.conf";
}
...
$sql = "
	SELECT id,mailbox,src_server,src_auth,src_user,src_password,src_folder,fetchall,keep,protocol,extra_options,usessl, sslcertck, sslcertpath, sslfingerprint
	FROM fetchmail
...
my (%config);
map{
	my ($id,$mailbox,$src_server,$src_auth,$src_user,$src_password,$src_folder,$fetchall,$keep,$protocol,$extra_options,$usessl,$sslcertck,$sslcertpath,$sslfingerprint)=@$_;
